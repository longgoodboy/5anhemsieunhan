Chào bạn, dựa trên tài liệu mẫu `02-deliverable-example.md` bạn đã cung cấp, tôi sẽ giúp bạn cấu trúc và chuyển đổi ý tưởng chủ đạo (**Ý tưởng số 2: Theo dõi và cập nhật deadline từ nhiều nguồn**) cùng hai ý tưởng còn lại thành một bản nộp **Day 02 — Weekly Report trước và sau AI** hoàn chỉnh, chuẩn chỉnh theo đúng format yêu cầu.
...
Dưới đây là bảng scan rộng các vấn đề dựa trên 3 ý tưởng gốc của bạn, được khai triển qua các lăng kính khác nhau để tìm ra dấu hiệu thực tế:
...
## Problem Card #1 — Theo dõi và cập nhật deadline (Ý tưởng chính)
...
### Draft current workflow (Dạng Mermaid viết trong Markdown)
...
## Research giải pháp (Tìm kiếm các hướng có sẵn)

| Nguồn / tool / case | Họ giải quyết phần nào? | Điểm mạnh | Khoảng trống / rủi ro | Bài học cho ứng dụng |
| --- | --- | --- | --- | --- |
| **Make.com / Zapier** | Tự động hóa kết nối Discord, Outlook sang Google Calendar. | Chạy tự động theo thời gian thực (Real-time). | Chỉ nhận diện được cấu trúc dạng Rule (nếu có form sẵn). Tin nhắn chat tự do của lớp trưởng không xử lý được. | Cần AI chen vào giữa để "đọc hiểu" đống tin nhắn tự do trước khi gửi cho Make/Zapier. |
| **Lark Base / Notion AI** | Trích xuất nội dung từ text thô thành bảng dữ liệu có ngày tháng. | Giao diện đẹp, dễ quản lý database. | Người dùng vẫn phải copy-paste thủ công từ Zalo/Outlook vào hệ thống. | Cần một khu vực đệm (Inbound) dễ dàng để quăng dữ liệu thô vào. |

Research takeaway:

> **Không nên kỳ vọng một Agent tự động log-in vào Zalo cá nhân** để đọc tin nhắn vì dính bảo mật và API hạn chế. Giải pháp thực tế nhất là một **Workflow**: Người dùng chụp ảnh/copy văn bản từ các nguồn quăng vào một "Hộp thư đến" (ví dụ: Group Telegram riêng/hoặc 1 công cụ AI), AI sẽ quét nội dung, trích xuất cấu trúc lịch và tự động đồng bộ lên Google Calendar thông qua 1 cú click chuột.

---

## Workflow before/after impact

| Metric | Trước (Thủ công) | Sau (Kỳ vọng với AI Workflow) | Ghi chú |
| --- | --- | --- | --- |
| **Tổng thời gian** | 60 - 80 phút | Dưới 10 phút | Tiết kiệm 85% thời gian |
| **Số bước xử lý** | 6 bước phân tán | 3 bước tập trung | Gom hết về 1 giao diện xử lý |
| **Rủi ro sai sót** | Cao (Gõ nhầm ngày/tháng) | Thấp (AI chuẩn hóa định dạng ngày) | Cần con người xác nhận lại bước cuối |
| **Bottleneck chính** | Ngồi rà soát và gõ lịch | Chỉ còn là bước Review nhanh | Thay đổi tính chất từ "làm việc chân tay" sang "giám sát chất lượng" |

---

## Problem Statement v1

* **Actor:** Sinh viên chưa quen với việc quản lý lượng thông tin deadline lớn từ nhiều nền tảng cùng lúc.
* **Workflow:** Thu thập thông tin từ Discord/Outlook/Zalo/Cổng thông tin -> Trích xuất thông tin deadline -> Lên danh sách nhắc nhở trên Google Calendar.
* **Bottleneck:** Đọc lọc thủ công từ tin nhắn trôi và gõ tay từng lịch hẹn mất thời gian, dễ nhầm lẫn.
* **Success Metric:** Giảm thời gian từ 60 phút xuống 10 phút/tuần; Không bị lỡ bất kỳ buổi thảo luận hay hạn nộp bài nào.
* **Boundary (Ranh giới AI):** AI không tự động tạo lịch khi chưa có sự xác nhận (approve) của bạn; AI không tự ý truy cập sâu vào tài khoản cá nhân khi chưa được cấp quyền bảo mật.
* **Mức chọn giải pháp:** **Workflow** kết hợp (Con người thu thập -> AI trích xuất cấu trúc -> Rule tự động đẩy lên Calendar thông qua API).

---

# 03 — Individual Reflection Example

## Bài học rút ra (Reflection)

* **Xác định đúng điểm nghẽn:** Bài toán "Deadline" thực chất không nằm ở việc tạo lịch (vì Google Calendar làm rất tốt), mà nằm ở bước **"đọc hiểu context"** của các tin nhắn tự do trên Zalo/Discord để biến nó thành dữ liệu có cấu trúc (Ngày, Giờ, Tên task). Đây chính là nơi AI (LLM) phát huy 100% sức mạnh của nó.
* **Tính khả thi của dữ liệu:** Nhận ra rằng các kênh đóng như Zalo rất khó để viết bot tự động quét tin nhắn do chính sách bảo mật, vì vậy thiết lập "Human Boundary" ở bước thu thập dữ liệu (mình chủ động copy các đoạn chat nghi ngờ có deadline quăng vào AI) là giải pháp thực tế nhất ở thời điểm hiện tại.
* **Hướng phát triển tiếp theo:** Nếu workflow này chạy ổn định, em có thể mở rộng sang ý tưởng số 1 (Tích hợp OCR quét ghi chú viết tay cuối tuần, trích xuất các task tự học trong sổ tay để nạp chung vào hệ thống quản lý lịch trình này).